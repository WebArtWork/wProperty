...
export class PropertiesrecordsComponent {
	...
	form: FormInterface = this._form.getForm('propertyrecord', propertyrecordFormComponents);

	config = {
		paginate: this.setRows.bind(this),
		perPage: 20,
		setPerPage: this._propertyrecordService.setPerPage.bind(this._propertyrecordService),
		allDocs: false,
		create: (): void => {
			this._form.modal<Propertyrecord>(this.form, {
				...
				click: async (created: unknown, close: () => void) => {
					...
					await firstValueFrom(
						this._propertyrecordService.create(created as Propertyrecord)
					);
				...
				buttons: [
					...
					},
					{
						...
						callback: async (): Promise<void> => {
							await firstValueFrom(this._propertyrecordService.delete(doc));
		...
		buttons: [
			{
				...
				click: (doc: Propertyrecord): void => {
					this._form.modalUnique<Propertyrecord>('propertyrecord', 'url', doc);
				},
			},
		],
		headerButtons: [
	...
	setRows(page = this._page): void {
		...
		this._core.afterWhile(
			this,
			() => {
				this._propertyrecordService.get({ page }).subscribe((rows) => {
				...
				.then(async (propertyrecords: Propertyrecord[]) => {
					if (create) {
						for (const propertyrecord of propertyrecords) {
							...
							await firstValueFrom(
								this._propertyrecordService.create(propertyrecord)
							);
						}
					} else {
						for (const propertyrecord of this.rows) {
							if (
								!propertyrecords.find(
									(localPropertyrecord) => localPropertyrecord._id === propertyrecord._id
								)
							) {
								await firstValueFrom(
									this._propertyrecordService.delete(propertyrecord)
								);
							}
						}
						...
						for (const propertyrecord of propertyrecords) {
							...
							if (localPropertyrecord) {
								this._core.copy(propertyrecord, localPropertyrecord);

								await firstValueFrom(
									this._propertyrecordService.update(localPropertyrecord)
								);
							} else {
								this._preCreate(propertyrecord);
	...
	private _preCreate(propertyrecord: Propertyrecord): void {
		delete propertyrecord.__created;
	}
}
